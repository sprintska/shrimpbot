...
parser.add_argument(
    ...
    type=str,
    default=os.path.abspath("test.vlb"),
)
parser.add_argument(
    "--db", help="sqlite db location", type=str, default="vlb_pieces.vlo"
)
parser.add_argument(
    "--purgedb", help="purge the database (BACKUP FIRST)", action="store_true"
)
...
def import_vlo_from(from_file=g_from_file, to_db=g_db_path):
    ...
    for entry in vlb_str.split(chr(27)):
        ...
        content = re.sub(
            "Table;\d{1,4};\d{1,4};", "Table;vlb_x_axis;vlb_y_axis;", content
        )
        catchall = ""
        ...
        if len(piecename) > 0:
            ...
            if piecetype == "shipcard":
                ship_token = ""
                if "quasar" in piecename:
                    ship_token = "quasarfirecruisercarrier"  # fuck the Quasar, I don't know why it can't be fucking normal
                else:
                    for line in entry.split("\t"):
                        if line.startswith("placemark;Spawn") and (
                            "Capital Ships" in line
                        ):
                            ship_token = line.split(
            ...
            if piecetype == "squadroncard":
                ...
                for line in entry.split("\t"):
                    if line.startswith("placemark;Spawn squadron"):
                        sqd_token = line.split("\\/VASSAL.build.widget.PieceSlot:")[
                            -1
                        ].split(";")[0]
            ...
            if exists_piece(conn, piecetype, piecename):
                print(
                    "[^]{} exists, updating it...".format(piecetype + "-" + piecename)
                )
                update_piece(conn, piecetype, piecename, content, catchall)
            else:
                print(
                    "[+]{} does not exist, creating it...".format(
                        piecetype + "-" + piecename
                    )
                )
...
if __name__ == "__main__":
    if dopurge:
        ...
        [print(x) for x in import_vlo_from(from_file=g_from_file, to_db=g_db_path)]
